...
class Video2(object):
    ...
    def advance_offset(self):
        ...
        print_system(str(self._VIDEO_ID), 'offset20161208: ' + str(self.frame_number))
        return
    ...
    def control_video(self, command_key):
        ...
        if command_key & 0xff == ord(str(self._VIDEO_ID)):
            ...
            if not self.flag_record:
                print_error(str(self._VIDEO_ID),
                            'cannot finish saving videos. type s and start to record video')
            else:
                self.flag_record = False
                self.video_number += 1
...
DATE = '20161215/'
DIRECTION = 'front/'
NAME = 'div_0_fC0005_000019.avi'
OFFSET = 0
##################################################


OVERLAP = 'overlap/'
FILENAME = 'data/' + DATE + DIRECTION + OVERLAP + NAME
SAVE_VIDEO_PATH = 'dev/divideo/' + DATE
SAVE_TEXT_PATH = 'dev/divtext/' + DATE
